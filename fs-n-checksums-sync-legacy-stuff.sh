...
cd './WORK_IN_PROGRESS' && \
mkdir -p './TV/Cartoons' && \
mv '../Выходные' './TV/Cartoons/Legacy Stuff' && \
find './TV' -type f >> "${SCRIPT_DIR}/files/files.txt" && \
mv './TV/Cartoons/Legacy Stuff' '../Выходные' && \
cd '..' && \
rm -r './WORK_IN_PROGRESS' && \
cd "${CHECKSUMS_DIR}" && \
find './db/sha1' -path './db/sha1/tv-legacy-stuff-*.sha1' -exec cat {} \; >> "${SCRIPT_DIR}/files/checksums.sha1" && \
cd "${SCRIPT_DIR}" && \
